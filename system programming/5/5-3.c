...
void tokenize(char **basket,char *commend)
{
	...
	while((tok=commend[seek]) != '\0') // send character address
	{
		if((seek == 0) && (tok != ' '))
	...
	seek = 0;
	while((tok = commend[seek]) != '\0') // insert '\0' on blank
	{
		if(tok == ' ')
			commend[seek] = '\0';
...
void insert_slash(char *no_slash)
{
	if(no_slash[strlen(no_slash)-1])
	{
		no_slash[strlen(no_slash)] = '/';
		no_slash[strlen(no_slash)+1] = '\0';
	}
}	
int main(int argc, char **argv)
{
	int i,pid,access_check=1;
	int i_path=0;
	char tok,buf[LENGTH];
	char tmp[LENGTH];
	char *basket[LENGTH];
	...
	{	
		...
		if(!basket[0])// situation only enter
			continue;
		if(!strcmp(basket[0],"setpath"))
		{	
			if(!basket[1])
				continue;	
			insert_slash(basket[1]);
			path[i_path]=malloc(strlen(basket[1]));
			strcpy(path[i_path],basket[1]);
			i_path++;
		}
		else
		{
